...
| `obr_efo` | observation | `cb_headroom`, `psnfl_trajectory` | PDF exec summary, discovered from `obr.uk/efo` → newest EFO download. **`fetchVia:"relay"`** (obr.uk 403s Workers egress); ingested via `POST /admin/relay-artefact` | event | **no — always human review** | off · shadow · relay |
