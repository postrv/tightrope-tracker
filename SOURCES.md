...
`apps/curator/src/sources/registry.ts`. Last regenerated on **2026-07-03**
(post adapter retirement + curator go-live). Re-run the audit if you add or
retire an adapter or a capture spec.
...
|---------|------|--------------|----------------------|---------|---------------------------------|-----------------------|
| `sp_global_pmi` | observation | `services_pmi` | HTML mirror (Trading Economics) of the S&P Global UK Services PMI **final** — the canonical S&P index/press pages return 403 to a server-side fetch, so the mirror is cited as a mirror, never the primary | monthly | yes (after shadow) | off · shadow |
| `gfk_confidence` | observation | `consumer_confidence` | HTML, NIQ (formerly GfK) consumer-confidence barometer landing → monthly article | monthly | yes | off · shadow · follow-link pending |
| `rics_rms` | observation | `rics_price_balance` | HTML landing → monthly PDF, RICS UK Residential Market Survey | monthly | yes | off · shadow · follow-link pending |
| `mhclg_housing` | observation | `housing_trajectory`, `planning_consents` | HTML gov.uk collection → quarterly release page (**not** the ODS attachments); formulas per `housing-history.json` | quarterly | yes, tight G4 (Δ≤30%) | off · shadow · follow-link pending |
| `obr_efo` | observation | `cb_headroom`, `psnfl_trajectory` | HTML `obr.uk/efo` → exec-summary PDF; event-driven via the daily hash poll | event | **no — always human review** | off · shadow |
| `ons_dd_failure` | observation | `dd_failure_rate` | HTML, ONS monthly Direct Debit failure-rate dataset page (extract from HTML, not the xlsx) | monthly | yes | off · shadow |
