/**
 * Shared admin-token gate (SEC-13): constant-time token compare + per-IP
 * exponential backoff. ONE implementation, consumed by both the ingest and
 * curator workers.
 *
 * History: this machinery grew up inside apps/ingest (timingSafeEqual in
 * admin.ts, the backoff in lib/adminBackoff.ts). The curator worker needs the
 * exact same gate on its review endpoints, so rather than copy it (and risk
 * the two drifting), it lives here. The ingest lib files are now thin
 * re-exports; behaviour and tests are preserved verbatim.
 *
 * Dependency-free by design: this package does not depend on
 * @cloudflare/workers-types, so the KV surface is a structural subset (the
 * three methods this gate actually calls). Both `KVNamespace` (ingest/curator
 * Env.KV) and a test double satisfy it. `Request`/`Response`/`TextEncoder` are
 * DOM globals surfaced by the base tsconfig lib.
 */
...
/**
 * Minimal structural KV surface the gate touches — deliberately not
 * @cloudflare/workers-types' `KVNamespace` (which this package must not
 * depend on). A `KVNamespace` is structurally assignable; so is a test double.
 */
export interface AdminGateKv {
  get(key: string): Promise<string | null>;
  put(key: string, value: string, options?: { expirationTtl?: number }): Promise<void>;
  delete(key: string): Promise<void>;
}

/** The gate only needs the KV binding off the worker's Env. */
...
async function readState(env: AdminGateEnv, ip: string): Promise<BackoffState> {
...
async function writeState(env: AdminGateEnv, ip: string, state: BackoffState, ttlSeconds: number): Promise<void> {
 ...
 * earlier failures).
 */
export async function clearFailures(env: AdminGateEnv, ip: string): Promise<void> {
 ...
 * comparison stays owned by the caller (which holds the secret in `Env`).
 */
export interface AdminAuthDeps {
  /** Returns true when the provided token matches the configured secret. */
