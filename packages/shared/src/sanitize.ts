 ...
 * attacker who controls an adapter response (or an unrecognised cron string)
 * can:
 ...
 *
 * Home of the SEC-14 helper: it lives here in `@tightrope/shared` so the
 * ingest DLQ/cron-miss paths and the curator cron-miss path apply ONE
 * implementation. `apps/ingest/src/lib/sanitize.ts` is a thin re-export.
 */
